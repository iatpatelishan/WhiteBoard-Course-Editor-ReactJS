...
const generateOptions = (topicId, widget, element, changeAnswerList) => {
                ...
                return (
                    <div className="form-check">
                        <input className="form-check-input" type="checkbox"
                               checked={element.answerList.indexOf(i) > -1}
                               name={"T" + topicId + "Elem" + element.orderno}
                               id={"T" + topicId + "Elem" + element.orderno + "Checkbox" + i + "Answer"}
                               ref={node => elemNode = node}
                               key={i} label={line} value={line} onChange={() => changeAnswerList(widget.id, element.id, i, elemNode.checked)} />
                        <label className="form-check-label" htmlFor={"T" + topicId + "Elem" + element.orderno + "Checkbox" + i + "Answer"}>
                            {line}
                        </label>
                    </div>);
            });
...
const CheckboxFormElement = ({topicId, widget, element, changeLabel, changeLabelDirection, changeCSS, changeOptions, changeAnswerList}) => {
    let labelElem;
    let labelDirection;
    let cssElem;
    let optionsElem;
    let formList = generateOptions(topicId, widget, element, changeAnswerList);


    return (
        <div>
            <hr/>
            <h4>Checkbox Element Editor</h4>
                ...
                <div className="col-sm-10">
                    ...
                    <div>
                        ...
                        <pre className="wbdv-cursor" contentEditable="true" onKeyDown={(e) => preventEditing(e)}>
                            ...
                            <br/>
                            <br/>
                            #{"T" + topicId + "Elem" + element.orderno + "Checkbox" + "0"} {"{"} <br/>
...
const dispathToPropsMapper = dispatch => ({
    ...
    changeAnswerList: (widgetId, elementId, opt, ans) => actions.changeAnswerListFormElement(dispatch, widgetId, elementId, opt, ans),
});
