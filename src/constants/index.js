export const MOVE_UP = 'MOVE_UP'
export const MOVE_DOWN = 'MOVE_DOWN'
export const SAVE = 'SAVE'
export const SAVE_WIDGETS = 'SAVE_WIDGETS'
export const FIND_ALL_WIDGETS = 'FIND_ALL_WIDGETS'
export const DELETE_WIDGET = 'DELETE_WIDGET'
export const CLONE_WIDGET = 'CLONE_WIDGET'
export const ADD_WIDGET = 'ADD_WIDGET'
export const SET_TEXT_WIDGET = 'SET_TEXT_WIDGET'
export const SET_WIDGET_TYPE = 'SET_WIDGET_TYPE'
export const PREVIEW = 'PREVIEW'
export const TOGGLE_PREVIEW = 'TOGGLE_PREVIEW'
export const ENABLE_EDITING = 'ENABLE_EDITING'
export const TOGGLE_EDITING = 'TOGGLE_EDITING'
export const WIDGET_NAME_CHANGED = 'WIDGET_NAME_CHANGED'
export const HEADING_TEXT_CHANGED = 'HEADING_TEXT_CHANGED'
export const HEADING_SIZE_CHANGED = 'HEADING_SIZE_CHANGED'
export const PARAGRAPH_TEXT_CHANGED = 'PARAGRAPH_TEXT_CHANGED'
export const LIST_ITEM_CHANGED = 'LIST_ITEM_CHANGED'
export const LIST_TYPE_CHANGED = 'LIST_TYPE_CHANGED'
export const IMAGE_URL_CHANGED = 'IMAGE_URL_CHANGED'
export const LINK_TEXT_CHANGED = 'LINK_TEXT_CHANGED'
export const LINK_URL_CHANGED = 'LINK_URL_CHANGED'
export const FORM_ADD_ELEMENT = 'FORM_ADD_ELEMENT'
export const ADD_FORM_INPUT = 'ADD_FORM_INPUT'
export const ADD_FORM_CHECK = 'ADD_FORM_CHECK'
export const ADD_FORM_RADIO = 'ADD_FORM_RADIO'
export const ADD_FORM_SELECT = 'ADD_FORM_SELECT'
export const ADD_FORM_TEXTAREA = 'ADD_FORM_TEXTAREA'